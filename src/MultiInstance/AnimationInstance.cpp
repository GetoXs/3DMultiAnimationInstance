...
//-----------------------------------------------------------------------------
HRESULT AnimationInstance::Setup( LPD3DXANIMATIONCONTROLLER pAC )
{
	assert( pAC != NULL );
...
//-----------------------------------------------------------------------------
void AnimationInstance::UpdateFrames( D3DXFRAME* pFrame, D3DXMATRIX* pmxBase )
{
	assert( pFrame != NULL );
	assert( pmxBase != NULL );
...
//       we find it.
//-----------------------------------------------------------------------------
void AnimationInstance::DrawFrames( D3DXFRAME* pFrame )
{
...
//-----------------------------------------------------------------------------
void AnimationInstance::DrawMeshFrame( D3DXFRAME* pFrame )
{
	AnimationMeshContainer* pMC = ( AnimationMeshContainer* )pFrame->pMeshContainer;
	D3DXMATRIX mx;
	...
	for( dwAttrib = 0; dwAttrib < pMC->m_dwNumAttrGroups; ++ dwAttrib )
	{
		...
		// the texture here
		m_pMultiAnim->m_pEffect->SetTexture( "texScene", pMC->m_apTextures[ pBC[ dwAttrib ].AttribId ] );
...
//-----------------------------------------------------------------------------
AnimationInstance::AnimationInstance( AnimationContainer* pMultiAnim ) : m_pMultiAnim( pMultiAnim ),
														 m_pAC( NULL )
{
...
//-----------------------------------------------------------------------------
AnimationContainer* AnimationInstance::GetMultiAnim()
{
	return m_pMultiAnim;
}
...
//-----------------------------------------------------------------------------
void AnimationInstance::GetAnimController( LPD3DXANIMATIONCONTROLLER* ppAC )
{
	assert( ppAC != NULL );
	m_pAC->AddRef();
	*ppAC = m_pAC;
...
//-----------------------------------------------------------------------------
D3DXMATRIX AnimationInstance::GetWorldTransform()
{
	return m_mxWorld;
}
...
//-----------------------------------------------------------------------------
HRESULT AnimationInstance::AdvanceTime( DOUBLE dTimeDelta, ID3DXAnimationCallbackHandler* pCH )
{
	HRESULT hr;
...
//-----------------------------------------------------------------------------
HRESULT AnimationInstance::ResetTime()
{
...
//-----------------------------------------------------------------------------
HRESULT AnimationInstance::Draw()
{
