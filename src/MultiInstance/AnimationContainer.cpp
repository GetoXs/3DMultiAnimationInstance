...
//-----------------------------------------------------------------------------
HRESULT AnimationContainer::CreateInstance( AnimationInstance** ppAnimInstance )
{
	*ppAnimInstance = NULL;

	LPD3DXANIMATIONCONTROLLER pNewAC = NULL;
	HRESULT hr;
	AnimationInstance* pAI = NULL;
	...
	{
		// create the new AI
		pAI = new AnimationInstance( this );
		if( pAI == NULL )
		{
			hr = E_OUTOFMEMORY;
			goto e_Exit;
...
//-----------------------------------------------------------------------------
HRESULT AnimationContainer::SetupBonePtrs( D3DXFRAME* pFrame )
{
	assert( pFrame != NULL );
	...
	{
		// call setup routine
		hr = ( ( AnimationMeshContainer* )pFrame->pMeshContainer )->SetupBonePtrs( m_pFrameRoot );
		if( FAILED( hr ) )
			return hr;
	}
...
AnimationContainer::AnimationContainer() : m_pDevice( NULL ),
						   m_pEffect( NULL ),
...
{
	vector <AnimationInstance*>::iterator itCur, itEnd = m_v_pAnimInstances.end();
...
//-----------------------------------------------------------------------------
HRESULT AnimationContainer::Setup( LPDIRECT3DDEVICE9 pDevice,
						   WCHAR sXFile[],
						   WCHAR sFxFile[],
						   MeshHierarchy* pAH,
						   LPD3DXLOADUSERDATA pLUD )
{
	...
	hr = D3DXLoadMeshHierarchyFromX( sXFile,
									 0,
									 m_pDevice,
									 pAH,
									 pLUD,
...
//-----------------------------------------------------------------------------
HRESULT AnimationContainer::Cleanup( MeshHierarchy* pAH )
{
	if( m_amxWorkingPalette )
	{
		delete [] m_amxWorkingPalette;
...
//-----------------------------------------------------------------------------
LPDIRECT3DDEVICE9 AnimationContainer::GetDevice()
{
	m_pDevice->AddRef();
	return m_pDevice;
}

D3DXMATRIX* AnimationContainer::GetWorkingPalette()
{
	return m_amxWorkingPalette;
}
...
//-----------------------------------------------------------------------------
LPD3DXEFFECT AnimationContainer::GetEffect()
{
	if( m_pEffect )
		m_pEffect->AddRef();
...
//-----------------------------------------------------------------------------
DWORD AnimationContainer::GetNumInstances()
{
...
//-----------------------------------------------------------------------------
AnimationInstance* AnimationContainer::GetInstance( DWORD dwIndex )
{
...
//-----------------------------------------------------------------------------
float AnimationContainer::GetBoundingRadius()
{
...
//-----------------------------------------------------------------------------
HRESULT AnimationContainer::CreateNewInstance( DWORD* pdwNewIdx )
{
	// create the AI
	AnimationInstance* pAI;
...
//-----------------------------------------------------------------------------
void AnimationContainer::SetTechnique( char* sTechnique )
{
...
//-----------------------------------------------------------------------------
HRESULT AnimationContainer::Draw()
{
...
//-----------------------------------------------------------------------------
HRESULT AnimationMeshContainer::SetupBonePtrs( D3DXFRAME* pFrameRoot )
{
	if( pSkinInfo )
	{
